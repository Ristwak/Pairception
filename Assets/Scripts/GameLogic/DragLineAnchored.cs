    ...
    public void OnEndDrag(PointerEventData data) {
        ...
        foreach (var r in results) {
            ...
            if (ab != null) {
                ...
                bool ok = QAMatchGameManager.Instance.EvaluateMatch(
                    _qb.questionText, ab.answerText);
                _lineImg.color = ok ? correctColor : wrongColor;
                matched = true;
                _qb.answered = true;
                break;
            }
        }
        if (!matched) Destroy(_lineRect.gameObject);
        _lineRect = null;
    }
